...
from util.RL.GYM_PPO_S_mau.TrafficSignalEnv import TrafficSignalEnv
...
class RLTrainer:
    ...
    def train_policy(self, iteration: int):
        ...
        with torch.no_grad():
            for step in range(num_steps):
                # 选择动作（批量）
                action, log_prob = self.agent.select_actions(obs, deterministic=False)
                # 执行动作（并行）
                ...
                # 一次性存入整个时间步的所有环境数据
                self.agent.trajectory_buffer.push(
                    state=obs,  # shape: (num_envs, state_dim)
                    action=action,  # shape: (num_envs, action_dim)
                    log_prob=log_prob,  # shape: (num_envs,)
                    reward=reward,  # shape: (num_envs,)
                    next_state=next_obs,  # shape: (num_envs, state_dim)
                    done=dones,  # shape: (num_envs,)
                )
        ...
        print(f"   Buffer 大小：{len(self.agent.trajectory_buffer)}步")
        ...
        print(f"   Entropy: {train_stats.get('entropy_loss', 0):.4f}")
    ...
    def test(self, render: bool = False):
        ...
        while step < self.config['simulation_steps']:
            # 选择动作
            action, _ = self.agent.select_actions(obs, deterministic=True)
            # 环境交互
